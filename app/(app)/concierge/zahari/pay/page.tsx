    ...
    <div className="max-w-md space-y-6">
      <header>
        ...
        <p className="text-sm text-plum-900/60">
          Payments are confirmed by concierge (bank transfer / card invoice).
        </p>
      </header>

      {!sovereignPaid ? (
        <Card>
          <CardTitle>Sovereign Search · USD {Number(eng.sovereignSearchFeeUsd).toLocaleString()}</CardTitle>
      ...
      {sovereignPaid && !eng.activationPaidAt && (
        <Card>
          <CardTitle>Covenant Activation · USD {Number(eng.covenantActivationFeeUsd).toLocaleString()}</CardTitle>
          <CardSubtitle className="mt-2">
            Due when you and your match enter courtship.
          </CardSubtitle>
          <form action={requestActivationPayment} className="mt-4">
            <Button type="submit" variant="outline">
              Request activation invoice
            </Button>
          </form>
        </Card>
      )}
